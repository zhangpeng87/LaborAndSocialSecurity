...
namespace LaborAndSocialSecurity.Uploaders
{
    public class WorkerAttendanceUploader : Uploader<WorkerAttendance>
    {
        ...
        {
            QueryString = $"SELECT S.record_id, S.project_id, S.worker_id, S.device_id, S.record_time, S.type FROM zhgd_person.d_card_record S INNER JOIN zhgd_lw.f_worker W ON S.worker_id = W.worker_id INNER JOIN zhgd_lw.f_group G ON W.group_id = G.group_id INNER JOIN zhgd_lw.f_cooperator C ON G.cooperator_id = C.cooperator_id WHERE S.record_time BETWEEN '2021-04-10 00:00:00' AND '2021-04-10 23:59:59' AND C.project_id = { HjApiCaller.Project_id };";
